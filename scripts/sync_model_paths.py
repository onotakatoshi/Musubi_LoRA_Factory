...
def path_from_best(root: Path, keywords: Iterable[str], avoid: Iterable[str] = ()) -> Path:
    value = best_file(root, keywords, avoid)
...
def sync_known_roots(paths: dict, models_dir: Path, overwrite: bool) -> int:
    ...
    changed += set_if_found(paths, "zimage_dit", find_existing(z / "transformer" / "diffusion_pytorch_model-00001-of-00002.safetensors", path_from_best(z / "transformer", ["diffusion", "model"], ["vae", "text_encoder"])), overwrite=overwrite)
    changed += set_if_found(paths, "zimage_vae", find_existing(z / "vae" / "diffusion_pytorch_model.safetensors", path_from_best(z / "vae", ["vae", "diffusion"])), overwrite=overwrite)
    changed += set_if_found(paths, "zimage_text_encoder", find_existing(z / "text_encoder" / "model-00001-of-00003.safetensors", path_from_best(z / "text_encoder", ["model", "text", "encoder"])), overwrite=overwrite)

    wan_t2v = models_dir / "wan" / "Wan2.2-T2V-A14B"
    changed += set_if_found(paths, "wan_vae", find_existing(wan_t2v / "Wan2.1_VAE.pth", path_from_best(wan_t2v, ["vae"], ["dit", "diffusion"])), overwrite=overwrite)
    changed += set_if_found(paths, "wan_t5", find_existing(wan_t2v / "models_t5_umt5-xxl-enc-bf16.pth", path_from_best(wan_t2v, ["t5", "umt5"])), overwrite=overwrite)
    changed += set_if_found(paths, "wan_dit", find_existing(wan_t2v / "low_noise_model" / "diffusion_pytorch_model.safetensors.index.json", path_from_best(wan_t2v / "low_noise_model", ["diffusion", "model"])), overwrite=overwrite)
    changed += set_if_found(paths, "wan_dit_high_noise", find_existing(wan_t2v / "high_noise_model" / "diffusion_pytorch_model.safetensors.index.json", path_from_best(wan_t2v / "high_noise_model", ["diffusion", "model"])), overwrite=overwrite)

    wan_i2v = models_dir / "wan" / "Wan2.2-I2V-A14B"
    changed += set_if_found(paths, "wan22_i2v_vae", find_existing(wan_i2v / "Wan2.1_VAE.pth", path_from_best(wan_i2v, ["vae"], ["dit", "diffusion"])), overwrite=overwrite)
    changed += set_if_found(paths, "wan22_i2v_t5", find_existing(wan_i2v / "models_t5_umt5-xxl-enc-bf16.pth", path_from_best(wan_i2v, ["t5", "umt5"])), overwrite=overwrite)
    changed += set_if_found(paths, "wan22_i2v_dit", find_existing(wan_i2v / "low_noise_model" / "diffusion_pytorch_model.safetensors.index.json", path_from_best(wan_i2v / "low_noise_model", ["diffusion", "model"])), overwrite=overwrite)
    changed += set_if_found(paths, "wan22_i2v_dit_high_noise", find_existing(wan_i2v / "high_noise_model" / "diffusion_pytorch_model.safetensors.index.json", path_from_best(wan_i2v / "high_noise_model", ["diffusion", "model"])), overwrite=overwrite)

    wan_ti2v = models_dir / "wan" / "Wan2.2-TI2V-5B"
    changed += set_if_found(paths, "wan22_ti2v_vae", find_existing(wan_ti2v / "Wan2.2_VAE.pth", wan_ti2v / "Wan2.1_VAE.pth", path_from_best(wan_ti2v, ["vae"], ["dit", "diffusion"])), overwrite=overwrite)
    changed += set_if_found(paths, "wan22_ti2v_t5", find_existing(wan_ti2v / "models_t5_umt5-xxl-enc-bf16.pth", path_from_best(wan_ti2v, ["t5", "umt5"])), overwrite=overwrite)
    changed += set_if_found(paths, "wan22_ti2v_dit", find_existing(wan_ti2v / "diffusion_pytorch_model.safetensors.index.json", path_from_best(wan_ti2v, ["diffusion", "model", "dit"], ["vae", "t5", "text_encoder"])), overwrite=overwrite)

    wan21 = models_dir / "wan" / "Wan2.1-T2V-14B"
    changed += set_if_found(paths, "wan21_vae", find_existing(wan21 / "Wan2.1_VAE.pth", path_from_best(wan21, ["vae"], ["dit", "diffusion"])), overwrite=overwrite)
    changed += set_if_found(paths, "wan21_t5", find_existing(wan21 / "models_t5_umt5-xxl-enc-bf16.pth", path_from_best(wan21, ["t5", "umt5"])), overwrite=overwrite)
    changed += set_if_found(paths, "wan21_dit", find_existing(wan21 / "diffusion_pytorch_model.safetensors.index.json", path_from_best(wan21, ["diffusion", "model", "dit"], ["vae", "t5"])), overwrite=overwrite)
