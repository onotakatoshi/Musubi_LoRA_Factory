...
def _target_hints(lora_type: str) -> list[str]:
    concepts = _split_triggers(lora_type)
    if not concepts:
        return []
    hints: list[str] = []
    for concept in concepts:
        if concept in TARGET_HINTS:
            hints.extend(TARGET_HINTS[concept])
        elif concept in GENERIC_TYPES_WITHOUT_TARGET_CHECK:
            continue
        else:
...
def _noise_words(lora_type: str) -> list[str]:
    noise: list[str] = []
    for concept in _split_triggers(lora_type):
        noise.extend(NOISE_WORDS.get(concept, []))
    return list(dict.fromkeys(noise))
...
def diagnose_captions(dataset_dir: Path, lora_type: str = "eye", lang: str = "日本語") -> str:
    ...
    if ja:
        lines = [
            "# Caption診断",
            "",
            f"Trigger words: {', '.join(_split_triggers(lora_type)) or '(未指定)'}",
            f"画像数: {len(imgs)}",
            ...
            f"Trigger wordが見つからない可能性: {len(target_missing)}",
        ...
        lines = [
            "# Caption Diagnostics",
            "",
            f"Trigger words: {', '.join(_split_triggers(lora_type)) or '(not set)'}",
            f"Images: {len(imgs)}",
            ...
            f"Too long: {len(long)}",
            f"Possibly missing trigger words: {len(target_missing)}",
    ...
    add("Trigger wordが見つからない可能性", "Possibly missing trigger words", target_missing)
